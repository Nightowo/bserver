...
// 引入组件
const Contextmenu = defineAsyncComponent(() => import('/@/layout/navBars/tags/contextMenu.vue'));
...
onBeforeMount(() => {
  ...
  Emitter.on('onCurrentContextmenuClick', (data: RouteItem) => {
    // 通过方法点击关闭 tagsView
    data.isFnClick = true;
...
onBeforeRouteUpdate(async (to) => {
  ...
  addTagsView(to.path, <RouteToFrom>to);
  getTagsRefsIndex(getThemeConfig.value.isShareTagsView ? state.routePath : state.routeActive);
});
